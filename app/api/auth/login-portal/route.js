...
import { verifyClienteCredentials } from "@/lib/repos";
...
export async function POST(request) {
  ...
  const user = await verifyClienteCredentials(email, password);
  if (!user) {
    return NextResponse.json(
      { error: "Credenciales inválidas o portal no activado." },
      { status: 401 }
    );
  }
