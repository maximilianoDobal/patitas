...
export async function verifyStaffCredentials(email, password) {
  const { rows } = await query(
    `SELECT id, nombre, email, rol, sucursal_id, password_hash FROM usuarios
     WHERE lower(email) = lower($1) AND rol IN ('recepcionista', 'veterinario')`,
    [email]
  );
  const user = rows[0];
  if (!user) return null;
...
export async function createTurno(input, session) {
  ...
  await query(
    `INSERT INTO turnos (
      ...
      fecha, hora_inicio, duracion_minutos, estado, notas_recepcion, creado_por
    ) VALUES ($1,$2,$3,$4,$5,$6,$7::date,$8::time,$9,$10,$11,$12)`,
    [
      turno.id,
      turno.sucursalId,
      turno.mascotaId,
...
export async function listClientes(sucursalId) {
  const { rows } = await query(
...
export async function getCatalog(session) {
  ...
  const [
    ...
    query(
      `SELECT u.id, u.sucursal_id, u.nombre, u.email, u.telefono, c.dni
       ...
       WHERE u.sucursal_id = $1 ORDER BY u.nombre`,
      [sucursalId]
    ),
  ...
  return {
    sucursal: suc
      ? {
          id: suc.id,
          codigoInterno: suc.codigo_interno,
          nombreComercial: suc.nombre_comercial,
