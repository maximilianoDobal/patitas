...
import { notifyTurnoCreated, notifyTurnoUpdated } from "@/lib/email/turnoNotify";
...
export async function createTurno(input, session) {
  ...
  await assertTurnoEnHorarioSucursal(turno);
...
export async function updateTurno(id, patch, session) {
  ...
  const reprogrammed =
    patch.fecha !== undefined ||
    patch.horaInicio !== undefined ||
    patch.tipoServicioId !== undefined ||
    patch.duracionMinutos !== undefined;
  if (reprogrammed) {
    await assertTurnoEnHorarioSucursal(updated);
  }
  ...
  void notifyTurnoUpdated({ before: current, after: updated });
  return updated;
}
